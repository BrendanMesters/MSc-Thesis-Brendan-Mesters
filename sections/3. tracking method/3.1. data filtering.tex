...
This noise occurs in \textit{streaks}, lines of points through 3d space. 
...
Many sophisticated systems for pointcloud filtering already exist \cite{ester1996dbscan, faisal2017density}.
These existing systems expect a high point density, since they are built for Lidar, which makes them unsuitable for mmWave radar pointclouds.
...
This filter is built up of two parts: a temporal filter, measuring the density between frames, and a spatial filter, measuring the density within a frame.
Figure \ref{figure: combined filtering} conceptually shows this process, with each row representing a different frame. 
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/internal data/Thesis combined filter diagram with arrows high.png}
    \caption{Combined filtering: output of the three filters on various frames}
...
This point weight comes from different point value assigning systems, such as the one described in \cref{sub-section: tracking method - data enhancement - point weight}.
...
By constructing the \textit{density score} from the weights of nearby points, we ensure that points that are already known to be important have a larger sway on the outcome.
The spatial filter works well against sparse noise, but does not function well against dense, single-frame noise spikes.
...
The temporal filter achieves this in a very similar way to the spatial filter, but it only considers points from previous frames.
A point's \textit{temporal score} is determined as the cumulative weight of nearby (with a distance less than 20 cm) points in the 3 previous frames.
Points in a sparse but consistent cluster will thus be given a relatively high temporal score, while dense clusters that only occur in one frame will get a very low temporal score.
This \textit{temporal score} is then compared to a \textit{temporal threshold} to determine if a point is \textit{signal} or \textit{noise}.
...
Due to the inconsistent data issue of mmWave sensors (described in \cref{subsection: background - millimeter wave radar - data characteristics}), the temporal filter may miss large amounts of data.
That is why it is important to use a combination of \textbf{both} the \textit{temporal} and the \textit{spatial} filter, in the form of a \textit{combined filter}.
...
We then introduce a \textbf{combination factor} $F$ that determines how much we take into account each of the filters.
A point is then considered signal if $F \times S_{spat} + (1 - F) \times S_{temp} > F \times T_{spat} + (1 - F) \times T_{temp} $.
If this is not the case, then the point is considered noise.
...
The combination factor, used to combine the outputs of the two filters.
It is important to have a clear strategy for how these variables will be tuned, since they all affect the output.
Our strategy tunes the spatial filter such that it identifies most actual signal points correctly, even if some noise still comes through.
The temporal filter is tuned such that it labels most actual noise points correctly, even if some signal gets labeled as noise.
The temporal filter will then be used to negate the false positives from the spatial filter.

To tune the individual filters, you can only change the distance at which to consider other points or the score threshold.
Increasing the threshold will increase the number of points that are labeled as noise, and decreasing it will do the opposite.
Increasing the distance will increase the number of points that are labeled as signal, but it also decreases the importance of tightly coupled clusters.
...
The combination factor starts at $0.5$ during tuning, and gets tuned up if too many signal points are missed, and tuned down if too much noise gets through.
...
A researcher would inspect the current working, note an error in the output, and would tune the filters (in accordance with the logic described above) in such a way as to try and mitigate this error.
