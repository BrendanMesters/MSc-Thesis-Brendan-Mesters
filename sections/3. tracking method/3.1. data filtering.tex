...
This noise often occurs in \textit{streaks}, lines of points through 3d space. 
These streaks are not very dense spatially and appear in only a single frame, see \cref{figure: noise streak}.
...
\begin{figure}[!htb]
    ...
    \caption{Overview of the internal workings of the combined noise filter}
    \label{figure: combined filtering}
\end{figure}

For these reasons, we propose a novel noise filter for low-density mmWave pointclouds.
...
The second column shows the \textit{temporal filter}, which considers the number of neighbours in \textbf{other frames}.
The third column shows the \textit{spatial filter}, which considers the number of neighbours in \textbf{the same frame}.
