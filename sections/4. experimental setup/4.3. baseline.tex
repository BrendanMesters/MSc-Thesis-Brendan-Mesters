...
\ednote{MARS is not "foundational", it is SOA}

For our comparison, we need a DL model that is a foundational work in the field of mmWave radar HPE, which ensures that both systems have comparable prior literature to build upon.
Furthermore, the DL model has to evaluate the same data that IAmMuse evaluates, the pointclouds of the mmWave radar.
...
With these considerations, the MARS model~\cite{an2021mars}, a foundational model in the field of mmWave HPE and one of the first systems to generate skeletal estimations out of sparse mmWave pointclouds, was chosen as the ideal comparison model.

The MARS paper and this thesis use similar hardware, both using a Kinect 2 for the ground truth and using mmWave radars from TI as input.
...
Furthermore, the design of the MARS model has no temporal context and estimates each pointcloud frame independently; therefore, it may struggle on sparse pointcloud frames.


\ednote{Either explain this in more detail here, or don't explain it here and do this in chapter 5.}
Since MARS is a DL model, it requires training to function; we thus consider multiple distinct trained versions of MARS for the evaluation.
% MARS, being a DL model, requires training in order to use it; for the evaluation, we consider multiple differently trained versions of MARS.
...
The specific training set and testing set for each of the considered models can be found in \cref{figure: MARS train test}.
