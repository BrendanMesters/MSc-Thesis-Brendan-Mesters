...
The first comparison that will be considered is with \textit{MARS baseline}, which is trained on the same data that IAmMuse is, for each recording.
...
% In this way, a new model will have to be trained for each recording that is being analysed, in the same way that IAmMuse has to initialise on separate data for separate recordings.
The IAmMuse system and this MARS model both train/initialise on the \textit{calibration data} and evaluate the \textit{usage data}, which means that these comparisons will be a fair "apples to apples" comparison.
% Since this model will be used as a baseline, the name of this MARS model is \textit{MARS baseline}.
% IAmMuse of course also initialised using this \textit{calibration data} and evaluated the \textit{usage data}, in this way, the baseline analysis will compare apples to apples, so to say.
...
% Show a few nasty frames of the visual replay.
In the comparison between IAmMuse and \textit{MARS baseline}, a significant discrepancy was observed.
The prediction accuracy of \textit{MARS baseline} marginally exceeded random performance, as can be seen in \cref{figure: a2a arm accuracy}.
This poor performance can be attributed to the limited size of the training set (ranging from 108 to 275 frames, depending on the specific recording), as opposed to the 24,066 frames used by the MARS paper ~\cite{an2021mars}.
% The small training set most likely causes that \textit{MARS baseline} uses, being between 108 and 275 frames, depending on the specific recording in question (see Appendix \ref{appendix: recording frame distribution} for an exhaustive list of frame counts).
% The MARS paper~\cite{an2021mars}, on the other hand, used \textit{24 thousand} frames for training.
IAmMuse avoids these limitations by using domain-specific knowledge in its design to avoid a dependence on large datasets.
% that was programmed into the system.
% If we take a look at the accuracy for predicting either both hands correct, or a single hand correct, we can see that \textit{MARS a2a} performs barely better than random, see \cref{figure: a2a arm accuracy}.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/results/Arm Accuracy baseline.png}
...
\begin{figure}[!htb]
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{figures/results/MARS a2a skeleton 1.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{figures/results/MARS a2a skeleton 2.png}
    \end{subfigure}
    
    \caption{Two skeletal predictions of MARS baseline (in red) with the ground truth as captured by the Kinect overlaid (in blue)}
    \label{figure: mars a2a skeletal estimation}
\end{figure}

The lack of training data can often produce low performance, or even unintelligible results with deep learning models.
This is in large part because there are no inherent concepts of logical bounds built into many DL models, similarly so with MARS.
This is even further illustrated by the fact that \textit{MARS baseline} is not able to generate correct-looking human skeletons, see \cref{figure: mars a2a skeletal estimation}.
This does, however, result in MARS performing very poorly when provided with the same tools as IAmMuse.
On the same training and testing data, the accuracy of single \textit{arm position} predictions is 2 times higher for IAmMuse, and the accuracy of the full \textit{pose state estimation} is more than 4 times higher for IAmMuse.
One inherent advantage of Deep Learning models, however, is that they can be trained with a large data set \textit{once} and used \textit{many times}.
This could alleviate the accuracy issues observed, at the cost of an increased training set size.
