...
Since the baseline MARS model suffered from a lack of data, which makes it act unpredictably, the next step is to introduce more training data to MARS.
For this purpose, two different models will be considered.
Firstly, the \textit{MARS pre-trained} model is considered, which is the precise model provided by \citeauthor{an2021mars} in their paper.
...
This should produce a reasonable result, as this is exactly what we want from it.
Secondly, MARS will be trained with the \textit{calibration data} from \textbf{all} recordings, which will be referred to as "\textit{calib trained}".
This should give the MARS model enough data to train on, while still only providing it with the same type of data that IAmMuse uses for its initialization.
...
Figure \ref{figure: pre-calib arm accuracy} shows how both the \textit{MARS pre-trained} and the \textit{MARS calib-trained} model perform only slightly better than random, both when considering the individual hand predictions, as well as the combined hand prediction.
While the accuracy here is roughly equivalent to that of the baseline MARS, the reason for this bad performance is very different.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/results/pre-calib arm accuracy.png}
    \caption{Accuracy of predicting one or both arms correctly for the IAmMuse system, the MARS pre-trained and the MARS calib-trained systems.}
    \label{figure: pre-calib arm accuracy}
\end{figure}

The baseline model was not able to generate successful human skeletons, leaving the output of its arm zone prediction to be relatively random.
The problem with \textit{MARS pre-trained} and \textit{MARS calib-trained}, however, is very different, as can be clearly seen in \cref{figure: a2a pre calib prediction likelihood}.
...
The baseline model (a2a, yellow) does not predict the various position states with the same likelihood, but overall, the various position states are all predicted at times (even if some are very rare).
This behaviour is to be expected, as the output of this model is, at least in some part, random. 
The MARS pre-trained (pink) and MARS calib-trained (orange) on the other seem to only predict one or two position states ever.
...
%     -[[ Analysis ]] -
This behaviour of the pre-trained and calib-trained models points to them suffering from something else.
...
Secondly, there are some significant differences in the hardware setup between MARS and IAmMuse.
...
In \cref{figure: skeletal estimate pre-trained}, it's shown how the skeletal estimate does not line up well, furthermore, the prediction almost always holds their arms slightly tilted downwards.

The calib-trained MARS model seems to suffer from a somewhat different issue, though.
The model always holds its arms out to the side, regardless of the input data, as can be seen in \cref{figure: skeletal estimate calib-trained}, the exact point prediction also rarely ever moves.
This indicates that this model has gotten stuck in a local maximum during training, since the position where the skeleton holds both its arms out straight minimizes the average mean squared error for \textit{calibration data}.
...
A lot of high-quality and diverse training data is needed to avoid models optimizing in the wrong way, e.g., minimizing their arms by holding their arms still, like the calib-trained model did.
Furthermore, one of the main advantages of DL models, being able to learn minute patterns in data, can become a downside when the measurement situation changes slightly.
