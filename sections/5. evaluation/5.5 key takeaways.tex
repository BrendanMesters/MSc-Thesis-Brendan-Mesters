...
\ednote{Add a section on \textit{Response Delay}\\ This section should show my response delay plots,\\ and (MAYBE )argue that a response delay is not really feasible to be calculated for MARS (due to jitter).\\ Discuss that this is not great, but that we produce an almost instant response almost half of the time.}
...
This was almost certainly caused by the stability issue with data from the mmWave Radar, which is discussed in \cref{section: background - millimetre wave radar}.
